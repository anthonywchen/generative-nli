...
class Tests(AllenNlpTestCase):
	...
	def test_gnli_with_iterator(self):
		tokenizer = GNLITokenizer.from_pretrained('roberta-large')
		...
		for batch in iterator(reader.read('data/mnli/dev.jsonl'), num_epochs=1):
			for i in range(batch['src'].size(0)):
				target = batch['target'].tolist()[i]
				premise_tokens = batch['metadata'][i]['premise_tokens']
				hypothesis_tokens = batch['metadata'][i]['hypothesis_tokens']
				target_length = batch['target_lengths'].tolist()[i]

				src = batch['src'].tolist()[i]
				prev_output_tokens = batch['prev_output_tokens'].tolist()[i]
				src_length = batch['src_lengths'].tolist()[i]

				# Test encoder and decoder input lengths
				assert len(src) <= max_premise_length
				assert len(target) <= max_hypothesis_length

				# Check conversion to ids
				...
				# Check padding
				assert tokenizer.convert_tokens_to_ids(['<pad>']*(len(src)-src_length)) == src[src_length:]
				assert tokenizer.convert_tokens_to_ids(['<pad>']*(len(target)-target_length)) == prev_output_tokens[target_length:]
				assert tokenizer.convert_tokens_to_ids(['<pad>']*(len(target)-target_length)) == target[target_length:]